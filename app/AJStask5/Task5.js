...
.controller('CtrlT5',['$scope', function ($scope) {
    $scope.home =
        [{
    ...
    $scope.arrOfRoom = function (k) { //возвращает массив с названием комнат

        let arr1 = [];
        for (let i = 0; i < $scope.home[k].rooms.length; i++) {
            arr1[i] = $scope.home[k].rooms[i].roomName;
        }
        return arr1
    ...
    $scope.masOfRooms = $scope.arrOfRoom(0);
    $scope.inputts5 = $scope.mas[0];
